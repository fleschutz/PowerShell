...
.EXAMPLE
	...
	⏳ Pinging...  host 'Laptop' ---20ms---> 'x.com' (IP 104.244.42.1)
.LINK
...
try {
	Write-Host "⏳ Pinging...  host '$(hostname)' ---" -noNewline
	...
	Write-Host "No reply from '$($hostname)' for 5sec - check the connection or maybe the host is down."
	exit 1
} catch {
